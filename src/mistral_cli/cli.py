...
@click.option(
    "--auto-confirm-safe",
    is_flag=True,
    help="Auto-confirm safe read-only commands.",
)
@click.option(
    "--confirm-all",
    is_flag=True,
    help="Skip all confirmations (trusted mode, use with caution).",
)
@click.option(
    "--max-iterations",
    default=10,
...
def agent(
    ...
    The agent can execute shell commands, read/write files, search code,
    and perform other actions to help accomplish tasks.

    All potentially dangerous operations require confirmation unless
    --confirm-all is specified.
    """
    from .agent import Agent, AgentConfig
    ...
    while True:
        try:
            ...
            try:
                ...
                # Response is displayed via on_response callback
            except KeyboardInterrupt:
                agent_instance.cancel()
                console.print("\n[yellow]Cancelled.[/]")
            except Exception as e:
